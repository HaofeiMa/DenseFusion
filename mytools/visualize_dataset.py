...
mask_dir = "/home/mahaofei/Documents/image_processing/segment-anything/notebooks/images/LINEMOD/ammeter/640_480/mask"
# mask_dir = "datasets/linemod/LINEMOD/data/01/mask"
mask_list = sorted(os.listdir(mask_dir))
mask_list = sorted(mask_list, key=lambda x: int(x.split(".")[0]))

# for item in range(1700):
for mask_file in mask_list:
    ...
    # 图像读取
    # image = Image.open('{0}/data/{1}/rgb/{2}.png'.format(root, '%02d' % obj, '%04d' % item))  # 读取图片
    # depth = np.array(Image.open('{0}/data/{1}/depth/{2}.png'.format(root, '%02d' % obj, '%04d' % item)))    # 读取深度
    # label = np.array(Image.open('{0}/data/{1}/mask/{2}.png'.format(root, '%02d' % obj, '%04d' % item)))    # 读取标签
    mask_path = os.path.join(mask_dir, mask_file)
    image_path = os.path.join(mask_dir.replace("mask", "rgb"), mask_file)
    depth_path = os.path.join(mask_dir.replace("mask", "depth"), mask_file.replace("jpg", "png"))
    image = Image.open(image_path)  # 读取图片
    depth = np.array(Image.open(depth_path))    # 读取深度
    label = np.array(Image.open(mask_path))    # 读取标签
    print(depth)
    if label.shape[-1] == 3:
    ...
    # image = cv2.imread('{0}/data/{1}/rgb/{2}.png'.format(root, '%02d' % obj, '%04d' % item))  # 读取图片
