...
estimator = PoseNet(num_points = 500, num_obj = 4)
estimator.cuda()
# refiner = PoseRefineNet(num_points = 500, num_obj = 13)
refiner = PoseRefineNet(num_points = 500, num_obj = 4)
refiner.cuda()
##################################################################################
...
videoWriter = cv2.VideoWriter('TestVideo.avi',fourcc,30,(640,480))#最后一个是保存图片的尺寸

# 获取图片数量
num_data = len(os.listdir(os.path.join(opt.data_root, "rgb")))
